...
<head>
	...
	$(document).ready(function(){
		...
		$("#closeproduction").click(function(){
			window.location.href = "/trazability";
		});
		
		
		$("#sendliters").click(function(){
			 sendDataToTableDescriptionLote();
			 var count = $("#countLoteCounter").val();
			 getLitersConsumedForPasteurizator(count);
		});
		
		
		$(".closemodal").click(function(){
			$("#modalHowLiters").hide();
			$("#modalHowLiters input").val("");
		...
		$("#endLote").click(function(){
			...
			if(confirm("¿Realmente quiere finalizar este lote?\nUna vez finalizado no podrá editarlo y el sistema le enviará al menú principal.")){
				...
				if(cuajoTemperature == "" || cuajoTime == "" || kgcheese == ""){
					...
					$.get('http://localhost:8080/trazability/service/setlotecounterlikefinished/' + loteNumber + '/' + 1 + '/' + idCuba , function(data){});
		...
		$("#loadLotePending").click(function(){
			...
			setTimeout(function(){
				getLitersConsumedForPasteurizator($("#countLoteCounter").val());	
			}, 1200);
	...
	function checkIfPasteurizatorIsRunningInLoteLoaded(){
	 ...
	 function sendExtraInformationAboutLote()
	 {
		 ...
		 $.get('http://localhost:8080/trazability/service/create/'+loteCounter+'/' + kgcheese+'/' + cuajedTime +'/' + cuajedTemperature , function(data){
			 alert("LOTE FINALIZADO CON ÉXITO" + data);
		 });
	 }
	 ...
	 function getLitersConsumedForPasteurizator(idActualLoteCounterForPasteurizatorValue){
		 ...
		 $.get('http://localhost:8080/trazability/lote/getlitersconsumed/' + idActualLoteCounterForPasteurizatorValue, function(data){
	...
	function listIngredients(){
	 ...
	 function sendIngredientSelected(){
		 ...
		 $.ajax({
				...
				error: function(xhr, text, error) 
				{
					...
					if(xhr.status == 403)
					{
						alert("ERROR AL ENVIAR INGREDIENTE");
					}
					if(xhr.status == 400)
					{
						alert("ERROR AL ENVIAR INGREDIENTE");
		...
		function sendDataToTableDescriptionLote(){
			 ...
			 var litersFilled            = $("#literstofill").val();
			 var loteNumber              = $("#countLoteCounter").val();
			 var idFilledFactoryTank     = $(".factoryTankSelected input[name=idfilled]").val();
			 var idFactoryTank           = $(".factoryTankSelected input[name=tankselected]").val();
			 var pasteurizator           = $("#pasteurizatorlist").val();
			 
			 var nameFactoryTank         = $(".factoryTankSelected input[name=factorytankname]").val();
			...
			tabla.push( new Array(loteNumber , idFactoryTank , idFilledFactoryTank , idOperator , idCuba , pasteurizator , litersFilled ));
			...
			setTimeout(function(){
				var count = $("#countLoteCounter").val();
				getDataLoteNotFinishedInTableReloaded(count);
			},1200);
		 ...
		 function sendLoteDataToBD(tablaSended)
		 {
			 $.ajax({
					...
					{ 
						...
						liters : tablaSended[0][6],
					},
					success: function(data) 
					{
						alert("LITROS ENVIADOS CON EXITO");
	 ...
	 function listFactoryTank()
	 {
		 ...
		 $.getJSON('http://localhost:8080/trazability/factorytank/listallfactorytank', function(data) {
			 ...
			 var i = 0;
			  ...
			  $.each(data, function(key, val)
			  {
				  ...
				  var tank = "<div id='factorytank" + i + "' class='factoryTankNoSelected'><input type='text' class='inputfactory' disabled='disabled' name='factorytankname' value='" + val.description+"'/>  <br><strong>CAPACIDAD ACTUAL</strong><input name='idfilled' type='hidden' class='inputwidth' value='" + val.idFilled + "'/>  </div>";                   
				  
		  		  $("#listfactoryTanks").append(tank);
		  		  
				  $("#factorytank" + i).append("<br><input type='text' disabled='disabled' class='inputfactory' id='actualLiters" + val.id + "'  /><input type='hidden' name='tankselected' value='"+val.id+"'  />");
				  ...
				  //EVENTO CLICK EN FACTORYTANK
			  	  $("#factorytank" + i).click(function() {
					  ...
					  if(className == "factoryTankNoSelected"){
						  ...
						  // NO BORRAR LA LLAMADA - Descomentar cuando funcione Arduino
						  //setLikeSelectedActiveFactoryTank(val.id);
						  $("#modalHowLiters").show();
						  $("#modalHowLiters input").val("");
					  }
					  ...
					  if(className == "factoryTankSelected"){
						  ...
						  // NO BORRAR LA LLAMADA - Descomentar cuando funcione Arduino
						  //setLikeNotSelectedActiveFactoryTank(val.id);
					  }
			  	  });
				  
				/* NO BORRAR Descomentar cuando funcione Arduino				  
		...
		function detachmentActualLoteCounterInPasteurizator(){
			...
			$.get('http://localhost:8080/trazability/pasteurizator/detachmentactualidlotecounterforworkinpasteurizator/' + 1 , function(data){
		...
		function loadLotesNotFinished(){
		 ...
		 {
			 $.get('http://localhost:8080/trazability/factorytank/setlikebloqued/'+id,function(data){
		 ...
		 {
			 $.get('http://localhost:8080/trazability/factorytank/setlikenotbloqued/' + id,function(data){
			 ...
			 $.getJSON('http://localhost:8080/trazability/lote/getdatalotenotfinished/'+loteCounter, function(data) {
				 ...
				 $("#operator").val(data[0].operator.id);
				  ...
				  $.each(data, function(key, val){
					  ...
					  var accionIconField = "<td><img  id='delete' name='"+val.id+"' class='icono' src='" + iconoDelete + "' /></td>";
					  
					  $("#tableMilkFilled").append("<tr>"+
							  "<td><strong>" + val.id + "</strong></td>" +
							  "<td>" + val.loteCounter.id + "</td>" +
						      "<td>" + val.factoryTank.description + "</td>" +
						      "<td>" + val.idFilledFactoryTank + "</td>" +
						      "<td>" + val.operator.nameOperator  + "</td>"+
						      "<td>" + val.cuba.description + "</td>" +
						      //"<td>" + val.pasteurizator.id  + "</td>"
						      "<td>" + val.liters + "</td>" +
						      "<td>" + accionIconField + "</td>" +
						      + "</tr>");
				  });
				  ...
				  $('img#delete').click(function(){
					  ...
					  if(confirm("¿Seguro de eliminar este elemento con IDENTIFICADOR->?"+idImagen)){

						  deleteItemInTableList(idImagen);
		...
		function getDataLoteNotFinishedInTableReloaded(loteCounter){
		...
		function deleteItemInTableList(id){
			$.get("http://localhost:8080/trazability/lote/delete/" +id , function(data){
...
<div class="container">
    ...
    <div class="separador"></div>
		    ...
		    <div class="row">
				<div class="span10 offset1">
				   <h3 class="text-center">P R O D U C C I Ó N  --- M A N U A L ---</h3>
			    	...
			    	<div class="span7">
				    	...
				    	<table class="table redondoysombra" id="tableHeader">
							<tr class="info">
								...
								<td class="success"><strong>Litros</strong></td>
								<td class="success"><strong>BORRAR</strong></td>
							</tr>
						</table>
   ...
   <!-- Ventana Modal -->
	...
	<div class="modal-header">
				  ...
				  <div class="modal-footer">
				    <button id="closeproduction" class="btn" data-dismiss="modal" aria-hidden="true">Cerrar</button>
				  </div>
	</div>
	
	
	
	<div id="modalHowLiters">
	<div class="modal-header">
	    <button type="button" class="closemodal" data-dismiss="modal" aria-hidden="true">x</button>
				    <h3 id="myModalLabel">ENVÍO DE LITROS A CUBA DE FÁBRICA</h3>
	</div>
				  <div class="modal-body">
				  <label>¿CUÁNTOS LITROS?</label>
				  <input type="text" id="literstofill" />
				  <button id="sendliters">ENVIAR LITROS</button>
				  </div>
				  
				  <div class="modal-footer">
				    <button class="btn closemodal" data-dismiss="modal" aria-hidden="true">Cerrar</button>
				  </div>
	</div>
